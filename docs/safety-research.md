...
|------|---------|------------|
| Fast resets | Firecracker snapshots (~28ms restore), Zeroboot COW forking (~0.8ms), mitos (live COW fork of a warm microVM, tens of ms), CubeSandbox (CubeCoW event-level snapshot/clone/rollback, <60ms creation), forkd (Firecracker fork server: children from a warm parent via kernel CoW, mid-execution branch), smolvm (CoW fork of a running libkrun machine) | Zeroboot is much faster but unproven — **known CSPRNG concern**: random state is shared across COW forks, so experiments may not be independent unless you explicitly reseed. This can silently invalidate results. mitos forks a *running* Firecracker microVM into N copy-on-write siblings and restores from memory snapshots in milliseconds — hardware-isolated and K8s-native, but very new (alpha); the same shared-random-state caveat applies to its COW forks. CubeSandbox (Tencent) brings the same CoW snapshot/clone/rollback pattern at v0.x maturity with an E2B-compatible API; the shared-random-state caveat presumably applies to its clones too — verify before relying on fork independence. forkd and smolvm bring running-state forks to a single host without Kubernetes — forkd's "100 children in ~101ms" and "~56ms live branch" are the author's figures; smolvm is CLI-first. The shared-random-state caveat applies to every CoW fork in this row. |
| GPU + parallelism | Modal (50k+ concurrent, GPU support), openkruise/agents (K8s, GPU memory checkpoint), Beam (GPU-attached sandboxes; AGPL self-hostable runtime) | Modal is cloud-only, closed source. openkruise/agents is self-hosted K8s with sandbox hibernation that preserves GPU memory state — useful for pausing and resuming training runs. Beam's sandboxes run on gVisor + runc (container tier, per Beam's own engineering posts rather than product docs) and can attach GPUs; the beta9 runtime can be self-hosted under AGPL-3.0. |
| Eval + RL framework | OpenSandbox (Alibaba), AgentENV (Moonshot AI / kvcache-ai) | OpenSandbox is explicitly designed for evaluation and RL training with multi-language SDKs; very new (created December 2025) — verify that RL support meets your specific needs before committing. AgentENV is the Firecracker environment platform built for Kimi K3's agentic RL, with an E2B-compatible API and sub-100ms incremental snapshot/fork; it is the only entry whose stated purpose is RL at frontier-lab scale, and its throughput and memory-overcommit figures come from the K3 technical report, not independent measurement. |
| Self-hosted scale | Firecracker + custom orchestration, mitos (K8s-native Firecracker), openkruise/agents (K8s), CubeSandbox (RustVMM/KVM, single-node to K8s), AgentENV (distributed Firecracker), k7 (Kata + Firecracker/QEMU or k7d, multi-node via Ansible) | Firecracker is battle-tested (powers AWS Lambda) but requires building your own orchestration — mitos supplies exactly that: a K8s operator, CRDs, and a KVM device-plugin for scheduling Firecracker microVMs (alpha, so not yet a production bet). openkruise/agents provides K8s-native lifecycle management with E2B API compatibility. CubeSandbox claims 2,000+ sandboxes per 96-vCPU host with <5MB per-sandbox overhead and scales from single-node to multi-node/K8s/Terraform — vendor benchmarks, v0.x. k7 offers three switchable VM backends and Cilium FQDN egress policies but is self-declared beta "undergoing security review". |
...
| Credential isolation | nono, cleanroom (Buildkite), sandcat (VirtusLab), CubeSandbox (credential vault), matchlock, shuru, clampdown, Declaw, InstaVM, Islo | All keep API keys outside the sandbox via a host-side / proxy boundary. nono adds L7 API endpoint filtering; cleanroom adds a microVM isolation boundary (stronger than nono's process-level); sandcat substitutes secrets at a transparent mitmproxy so a container running in bypass mode never sees real credential values (container-tier, so weaker containment than the other two). CubeSandbox pairs the vault pattern with hardware VM isolation at self-hosted scale. By August 2026 the pattern had become common: matchlock and shuru inject secrets at a host-side proxy into local microVMs; clampdown hands the agent a dummy API key at container tier; Declaw, InstaVM, and Islo do the same as hosted services (Islo's gateway rewrites tokens at egress). See docs/strategy-update-2026-09-03.md for the full set. Directly tests whether agents attempt credential exfiltration. |
...
| Observability / intervention | sevorix-lite, gVisor, loop, agent-glovebox, Leash | sevorix-lite's "Yellow Lane" dashboard lets researchers observe and intervene in real time (approve/deny/redirect). loop traps syscalls via seccomp RET_USER_NOTIF and routes each trap to a three-button Slack/Discord card with once/session/deny caching — same human-in-the-loop pattern, different operator surface (chat vs. dashboard). agent-glovebox adds a third surface: a second-model monitor gates flagged tool calls, pushes phone notifications, and can halt the agent remotely — the monitor is a best-effort filter, but the halt is real. gVisor logs denied syscalls for post-hoc analysis. Leash (StrongDM) records syscall telemetry from a Docker/Podman sandbox and evaluates Cedar policies over agent and MCP tool actions behind a web UI — the richest policy language in this table, in a repo idle since April 2026. |
| Reproducibility | Firecracker snapshots, Fly Sprites (persistent), Morph Cloud (Infinibranch) | Snapshot before evaluation, restore to replay. Fly Sprites offers persistent 100GB NVMe if you need durable eval environments. Morph Cloud branches a running VM's memory and disk into independent copies (vendor claims <250ms), so an evaluation can fork at a decision point and compare continuations; hosted and closed-source. |
...
| Need | Options | Trade-offs |
|------|---------|------------|
| Hardware VM containment | Firecracker (with jailer), Kata Containers, cleanroom (Buildkite), brood-box (Stacklok), CubeSandbox (Tencent), cocoon (Cloud Hypervisor, no NIC by default), matchlock (Firecracker / Virtualization.framework) | Separate kernel per sandbox. Firecracker's jailer adds defense in depth. cleanroom and brood-box both provide ready-to-use tools with hardware VM isolation on Linux and macOS. brood-box adds TOCTOU-resistant diff review — the VM is stopped before the user reviews changes, preventing the agent from modifying files during review. CubeSandbox pairs KVM microVMs with per-sandbox traffic tokens and policy-routing egress. cocoon removes the guest network device entirely and carries all I/O over vsock, shrinking the network attack surface to the host daemon — created July 2026, unaudited, AGPL server. matchlock is the local-laptop equivalent with proxy-injected secrets. |
| Escape detection | gVisor (denied syscall logging), sevorix-lite (eBPF syscall interception), nono (boundary violation recording), code-on-incus (nftables threat detection + auto-kill), agent-glovebox (second-model monitor + remote halt) | Most sandboxes silently block forbidden actions. gVisor, sevorix-lite, and nono record them. code-on-incus goes further — actively detects escape-like network behavior (reverse shells, C2 callbacks, DNS tunneling) and auto-pauses or kills the container. agent-glovebox watches at the tool-call layer instead of the syscall layer: a second model flags suspicious calls, alerts a human by phone, and can halt the session — semantic detection that syscall-level monitors can't do, at the cost of being best-effort rather than enforced. |
| Network monitoring | nono (L7 filtering + logging), cleanroom (deny-by-default egress + policy allowlists), sandcat (transparent WireGuard→mitmproxy over all HTTP/S, DNS, TCP/UDP), Anthropic srt (proxy with interactive approval), clawker (Envoy + CoreDNS deny-by-default with per-request path/method audit), sandlock (HTTP method/host/path ACLs at process tier) | See exactly what network endpoints the agent attempts to reach, including denied requests. cleanroom's deny-by-default posture means nothing gets through unless explicitly allowed. sandcat captures *all* container traffic transparently — no per-tool proxy config — with an allow/deny engine over the full flow. clawker and sandlock both filter at the HTTP request level (method, host, path) rather than the domain level: clawker via an Envoy MITM proxy in front of a Docker container, sandlock via a transparent MITM CA around a Landlock + seccomp process sandbox — the only process-tier entry with request-path egress rules. |
...
| agent-glovebox | MicroVM isolation (Docker sbx), egress allowlist, second-model monitor with remote halt, per-repo credential scoping, tamper-evident external audit log | ~10 weeks old at inclusion. The monitor and sanitization layers are self-described best-effort filters on top of the hard boundaries; coverage/mutation-testing claims are the project's own. |
| CubeSandbox | KVM microVM per sandbox, credential vault, CoW snapshot/clone/rollback, per-sandbox traffic tokens | v0.x (open-sourced April 2026) with a fast-moving surface. Performance and density figures are vendor benchmarks. Tencent-backed with ~10.9k stars, but the youngest hardware-VM entry here. |
| cplt | Kernel-primitive enforcement (Seatbelt/Landlock/seccomp), default-deny credential paths, egress proxy, committed tighten-only policy | Backed by NAV (Norwegian government). Linux enforcement is self-described as weaker than macOS (full network filtering needs kernel 6.7+); gh/git guards are soft barriers. |
| AgentENV | Distributed Firecracker environments, sub-100ms snapshot/fork, memory overcommit, E2B-compatible API | Built for and by a frontier lab's RL pipeline (Moonshot AI), which is a strong fitness signal and also means the public repo is a byproduct; scale figures are from the K3 technical report. |
| forkd | Firecracker fork server, CoW children from a warm parent, mid-execution branch, MCP server, audit logging | Solo maintainer; latency figures are the author's. Forks share random state unless reseeded. |
| sandlock | Landlock + seccomp + user-notify process sandbox, HTTP-level egress ACLs, copy-on-write filesystem, port virtualization | Org-maintained with a long commit history, but process-tier (shared kernel) and the seccomp user-notify resource limits are README claims not checked against code here. |
| matchlock | Firecracker / Virtualization.framework microVMs, MITM proxy secret injection, FUSE filesystem interception, runtime allowlist edits | Effectively one author (≈500 of ≈520 commits). MIT. Combines the VM tier with the credential-proxy pattern on a laptop, which nothing else here does. |
| vetto | Hand-built Landlock (ABI 1–6) + seccomp-BPF + namespaces, host-side DNS broker passing connected sockets into the sandbox, secret-path masking | Repository three weeks old at inclusion, one author, ~600 commits, no external audit. The isolation modules were read before listing and match the README; the macOS read-denial caveat is unverified. |
...
1. **Few sandboxes default to an adversarial posture.** Most assume the agent is trying to be helpful. cleanroom's deny-by-default egress and credential proxy come closest to treating the agent as potentially adversarial among the enforcement-only tools. agent-glovebox is the first entry to combine containment (microVM + egress allowlist) with active monitoring and alerting (a second-model monitor that flags tool calls, notifies a human, and can halt the session) — but the monitoring layer is best-effort semantic filtering, not enforcement, and the project is weeks old. A purpose-built safety research sandbox combining deny-by-default containment with *enforced* escape detection and behavioral alerting still doesn't exist.
